...
/*   Updated: 2023/10/23 21:12:03 by psoares-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

static void	proc_child(t_tree *node, t_cmd *cmd, int *fd, t_tree **root)
{
	char	*path;
	char	**cmds;
	(void )root;
...
static t_cmd	proc_exec_cmd(t_tree *node, char ***env, int pos, int cmd_num, t_tree **root)
{
	t_cmd		cmd;
	static int	fd;
...
void	proc_exec_tree(t_tree **root, char ***env)
{
	...
	{
		if (!(p == 1 && !(tmp->right)))
			cmd = proc_exec_cmd(tmp, env, p, cmd_num, root);
		if (p++)
			tmp = tmp->parent;
	}
